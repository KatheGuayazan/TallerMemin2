...
public class Firestore : MonoBehaviour
{
    ...
    async void Start()
    {
        db = FirebaseFirestore.DefaultInstance; // db reference
        await AddDataToCollection();
        await ReadAllData();
    }

    public async Task AddDataToCollection() // this method will add a document to the collection "users" with the id "memo"
    {
        DocumentReference docRef = db.Collection("users").Document("memo"); // db/users/memo
